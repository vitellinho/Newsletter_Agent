...
SITEMAP_INDEX_URL = "https://www.bankingclub.de/sitemap_index.xml"
SOURCE = "bankingclub"
...
def crawl_bankingclub(days_back: int = 7,
                      limit: int = 0,
                      sleep: float = 0) -> list[dict[str, Any]]:
...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="crawler_parser",
        description="Crawlt Artikel der letzten X Tage über den Sitemap-Index",
    )
...
if __name__ == "__main__":
    articles = crawl_bankingclub()
    path = save_bulk_json("articles", articles)
    print(f"[bankingclub] ✔ {len(articles)} Artikel gespeichert unter: {path}")